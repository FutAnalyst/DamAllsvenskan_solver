...
def create_squad_timeline(current_squad, statistics, picks, filename):
    ...
    text_color = "#ffffff"
    stats_color = "#a0a0a0"
    position_colors = {"GKP": "#4a1b7a", "DEF": "#0d4a6b", "MID": "#6b5c0d", "FWD": "#6b1d1d"}
    ...
    for gw_idx, week in enumerate(display_weeks):
        ...
        for _, player in starting_xi.iterrows():
            ...
            cell = patches.Rectangle(
                (gw_idx * gameweek_spacing - box_width / 2, y_pos - box_height / 2), box_width, box_height, facecolor=cell_bg_color, edgecolor="none"
            )
            ax.add_patch(cell)
            ...
            ax.text(gw_idx * gameweek_spacing, y_pos + 0.15, player["name"], color=text_color, ha="center", va="center", fontsize=8)
            stats_text = f"{player['xP']:.1f} xPts : {int(player['xMin'])} xMin"
            ax.text(gw_idx * gameweek_spacing, y_pos - 0.15, stats_text, color=stats_color, ha="center", va="center", fontsize=6)
            player_idx += 1
        for _, player in bench.iterrows():
            ...
            cell = patches.Rectangle(
                (gw_idx * gameweek_spacing - box_width / 2, y_pos - box_height / 2), box_width, box_height, facecolor=bench_bg_color, edgecolor="none"
            )
            ax.add_patch(cell)
        ...
        if week != base_week and week in statistics:
            ...
            ax.text(gw_idx * gameweek_spacing, stats_y, f"Lineup {gw_statistics['xP']:.2f} xPts", color=text_color, fontsize=10, ha="center")
            ax.text(gw_idx * gameweek_spacing, stats_y - 0.4, f"Obj {gw_statistics['obj']:.2f} xPts", color=text_color, fontsize=10, ha="center")
            ax.text(gw_idx * gameweek_spacing, stats_y - 0.8, f"ITB: {gw_statistics['itb']:.1f}", color=text_color, fontsize=8, ha="center")
